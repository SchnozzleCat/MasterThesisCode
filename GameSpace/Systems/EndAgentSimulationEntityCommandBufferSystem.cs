...
{
    /// <summary>
    /// This additional command buffer system is required at the end of each catch up step as an agent
    /// could have pending changes from a command buffer that influence the result of a future catch up step,
    /// but the changes code in question, and if possible a different command buffer system should be used to
    /// avoid the structural change at the end of each catch up step.
    ...
    public partial class EndAgentSimulationEntityCommandBufferSystem : EntityCommandBufferSystem
    {
        /// <summary>
